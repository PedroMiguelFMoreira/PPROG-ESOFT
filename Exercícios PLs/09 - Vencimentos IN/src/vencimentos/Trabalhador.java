...
public abstract class Trabalhador implements Comparable<Trabalhador> {
    ...
    /**
     * Compara o vencimento entre dois trabalhadores.
     * 
     * @param outroTrabalhador Vencimento do trabalhador com o qual se faz a comparação.
     * @return Resultado da comparação: 1, -1 ou 0.
     */
    @Override
    public int compareTo(Trabalhador outroTrabalhador) {
        if (this.vencimento() > outroTrabalhador.vencimento()) {
            return 1;
        } else if (this.vencimento() < outroTrabalhador.vencimento()) {
            return -1;
        }
